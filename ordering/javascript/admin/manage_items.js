document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadProducts() {
        try {
            ...
            const baseUrl = window.location.hostname.includes('esangdelicacies.com') 
                ? 'https://esangdelicacies.com' 
                : '';
            const response = await fetch(baseUrl + '/public/api/get_products_with_stock.php');
    ...
    const renderTable = (itemsToRender) => {
        ...
        itemsToRender.forEach(item => {
            ...
            saveStockBtn.addEventListener('click', async () => {
                ...
                try {
                    const response = await fetch('/public/api/update_product_stock.php', {
                        method: 'POST',
    ...
    editForm.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('/public/api/update_product.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    product_id: id,
    ...
    confirmDeleteBtn.addEventListener('click', async () => {
        ...
        if (idToDelete) {
            try {
                const response = await fetch('/public/api/delete_product.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
